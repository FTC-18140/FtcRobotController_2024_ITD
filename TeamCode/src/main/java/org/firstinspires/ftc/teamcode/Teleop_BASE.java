...
@TeleOp(group = "Teleop")
public class Teleop_BASE extends OpMode {
...
//    private ElapsedTime teleopTimer = new ElapsedTime();
    private Deadline hangTimer = new Deadline(110, TimeUnit.SECONDS);
    private Deadline gameOverTimer = new Deadline( 120, TimeUnit.SECONDS);
    private Deadline orangeLEDTimer = new Deadline( 105, TimeUnit.SECONDS);
...
//        if((teleopTimer.seconds() > 105 && Math.sin(2*teleopTimer.seconds()) > 0) || teleopTimer.seconds() > 120)
        if (useDeadlines && (orangeLEDTimer.hasExpired() || gameOverTimer.hasExpired()) )
        {
...
//        telemetry.addData("lift target position: ", robot.lift.lift_target);
        telemetry.addData("time: ", gameOverTimer.seconds());
